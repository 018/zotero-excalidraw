...
if (!Zotero.ZoteroExcalidraw.Notes) Zotero.ZoteroExcalidraw.Notes = {};

Zotero.ZoteroExcalidraw.Notes = Object.assign(Zotero.ZoteroExcalidraw.Notes, {
	init() {
		Zotero.ZoteroExcalidraw.Logger.log('Zotero.ZoteroExcalidraw.Notes inited.');
  ...
  statisticsToText(statistics) {
    ...
    let word_params = {
      ...
      cn_scale: statistics.length > 0 ? (Zotero.ZoteroExcalidraw.Utils.scale(statistics.cn_words / statistics.length, 1) + '%') : '-',
      en_words: statistics.en_words.toString(),
      en_scale: statistics.words > 0 ? (Zotero.ZoteroExcalidraw.Utils.scale(statistics.en_words / statistics.length, 1) + '%') : '-',
      num_words: statistics.num_words.toString(),
      num_scale: statistics.words > 0 ? (Zotero.ZoteroExcalidraw.Utils.scale(statistics.num_words / statistics.length, 1) + '%') : '-',
      other_words: statistics.words - statistics.cn_words - statistics.en_words - statistics.num_words,
      other_scale: statistics.words > 0 ? (Zotero.ZoteroExcalidraw.Utils.scale((statistics.words - statistics.cn_words - statistics.en_words - statistics.num_words) / statistics.length, 1) + '%') : '-',
    };
    ...
    let displayStore = Zotero.ZoteroExcalidraw.Utils.displayStore(statistics.sizes);
    return {
      text: word,
      title: Zotero.ZoteroExcalidraw.L10ns.getString('zotero-excalidraw-words-title', word_params),
      space: `${displayStore.text} (${Zotero.ZoteroExcalidraw.L10ns.getString('zotero-excalidraw-sizes-contentscale', word_params)}${statistics.sizes > 0 ? Zotero.ZoteroExcalidraw.Utils.scale(statistics.words / statistics.sizes, 1) : 0}%)`
    };
  },
  ...
  search: function (libraryID, collectionKey, savedSearchKey, excludeCollectionKeys) {
    Zotero.ZoteroExcalidraw.Logger.log({libraryID, collectionKey, savedSearchKey, excludeCollectionKeys});
    ...
    if (Zotero.ZoteroExcalidraw.Objects.isNoEmptyArray(excludeCollectionKeys)) {
  ...
  links(noteID) {
    ...
    if (note.parentItem) {
      links.push(...Zotero.ZoteroExcalidraw.Items.links(note.parentItemID));
    } else {
        let collectionID = note.getCollections()[0];
        links.push(...Zotero.ZoteroExcalidraw.Collections.links(collectionID));
    }
